...
export default function MarketplaceExperienceV3() {
  ...
  function handleCheckout() {
    ...
    const order = {
      ...
      items: cart.map((item) => ({ id: item.id, name: viewProduct(item, lang).name, quantity: item.quantity, price: item.price, store: item.store }))
    };
