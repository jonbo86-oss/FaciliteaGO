    ...
    <article className="group overflow-hidden rounded-[28px] bg-white shadow-xl transition hover:-translate-y-1 hover:shadow-2xl">
      <a href={productHref} className="block">
        ...
        <div className="p-5 pb-3">
          ...
          <div className="mt-4 flex items-center gap-3 text-sm font-bold text-slate-500"><span className="flex items-center gap-1"><Star className="h-4 w-4 fill-yellow-400 text-yellow-400" /> {product.rating}</span><span className="flex items-center gap-1"><Clock className="h-4 w-4" /> {product.pickup}</span><span>{t.stock} {product.stock}</span></div>
        </div>
      </a>
      <div className="flex items-end justify-between px-5 pb-5"><div>{product.oldPrice && <p className="text-sm font-bold text-slate-400 line-through">{money(product.oldPrice)}</p>}<p className="text-3xl font-black text-[#0072CE]">{money(product.price)}</p></div><button onClick={() => onAdd(product)} className="rounded-2xl bg-[#0072CE] px-4 py-3 text-sm font-black text-white">{t.add}</button></div>
    </article>
  );
}
...
export default function MarketplaceExperience() {
  ...
  const t = dict[lang];
  const quickPrompts = [t.promptReform, t.promptNearby, t.promptSortNear, t.promptCoupon];
  ...
  const filtered = useMemo(() => {
    ...
    const base = marketplaceProducts.filter((product) => {
      const translated = viewProduct(product, lang);
      const matchesCategory = category === "Todos" || product.category === category;
      const matchesQuery = !query || [translated.name, product.store, translated.category, product.district, translated.description].join(" ").toLowerCase().includes(normalized);
  ...
  function handleLogin(withCaixa = false) { const typedName = loginName.trim(); const finalName = typedName || (withCaixa ? "Cliente CaixaBank" : "Cliente"); const email = typedName.includes("@") ? typedName : `${finalName.toLowerCase().replace(/\s+/g, ".")}@faciliteago.com`; const user = { name: finalName, email }; setCurrentUser(user); window.localStorage.setItem("faciliteago-user", JSON.stringify(user)); setLoginName(""); setLoginPassword(""); setLoginOpen(false); }
  function handleCheckout() { if (cart.length === 0) return; const order = { id: `GO-${Date.now().toString().slice(-6)}`, status: lang === "ca" ? "Comanda confirmada" : "Pedido confirmado", store: cart.length === 1 ? cart[0].store : `${new Set(cart.map((item) => item.store)).size} ${lang === "ca" ? "comerços" : "comercios"}`, total: money(total), pickup: `PK-${Date.now().toString().slice(-4)}`, date: new Date().toLocaleDateString("es-ES"), items: cart.map((item) => ({ id: item.id, name: viewProduct(item, lang).name, quantity: item.quantity, price: item.price, store: item.store })) }; const existingOrders = JSON.parse(window.localStorage.getItem("faciliteago-orders") || "[]"); window.localStorage.setItem("faciliteago-orders", JSON.stringify([order, ...existingOrders])); setCart([]); setCoupon(""); window.localStorage.setItem("faciliteago-cart", JSON.stringify([])); setCartOpen(false); router.push("/usuario"); }
  function addToCart(product: MarketplaceProduct) { setCart((current) => { const found = current.find((item) => item.id === product.id); if (found) return current.map((item) => item.id === product.id ? { ...item, quantity: item.quantity + 1 } : item); return [...current, { ...product, quantity: 1 }]; }); setCartOpen(true); }
  function changeQuantity(id: string, delta: number) { setCart((current) => current.flatMap((item) => { if (item.id !== id) return [item]; const next = item.quantity + delta; return next <= 0 ? [] : [{ ...item, quantity: next }]; })); }
  function assistantReply(text: string) { const lower = text.toLowerCase(); let reply = t.defaultReply; if (lower.includes("reforma") || lower.includes("herramienta") || lower.includes("eina") || lower.includes("pinzas") || lower.includes("pinces")) { setQuery(lang === "ca" ? "ferreteria" : "ferreteria"); setCategory("Ferreteria"); reply = t.reformaReply; } else if (lower.includes("gominola") || lower.includes("llamin") || lower.includes("caramelo") || lower.includes("anillo") || lower.includes("anell")) { setQuery(lang === "ca" ? "llaminadures" : "gominolas"); setCategory("Gominolas"); reply = t.sweetsReply; } else if (lower.includes("cerca") || lower.includes("cercania") || lower.includes("proximitat")) { setSort("near"); reply = `${t.nearReply} ${location}.`; } else if (lower.includes("cupon") || lower.includes("cupó") || lower.includes("finbroadpeak26")) { setCoupon("FINBROADPEAK26"); reply = t.couponReply; } setMessages((current) => [...current, { role: "user", text }, { role: "assistant", text: reply }]); }
  ...
  return (
    <main className="min-h-screen bg-[#f4f8ff] text-slate-950">
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 pb-12 md:grid-cols-[260px_1fr] md:px-6"><aside className="rounded-[28px] bg-white p-5 shadow-xl md:sticky md:top-24 md:h-fit"><div className="mb-4 flex items-center gap-2 text-sm font-black text-[#002B5C]"><SlidersHorizontal className="h-5 w-5" /> {t.filters}</div><div className="space-y-2">{marketplaceCategories.map((item) => <button key={item} onClick={() => setCategory(item)} className={`w-full rounded-2xl px-4 py-3 text-left text-sm font-black ${category === item ? "bg-[#0072CE] text-white" : "bg-blue-50 text-[#002B5C]"}`}>{categoryLabel(item, lang)}</button>)}</div><div className="mt-5"><p className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-slate-500">{t.sort}</p><select value={sort} onChange={(event) => setSort(event.target.value)} className="w-full rounded-2xl border border-blue-100 bg-white px-4 py-3 text-sm font-bold outline-none"><option value="near">{t.near}</option><option value="price">{t.price}</option><option value="rating">{t.rating}</option></select></div></aside><section><div className="mb-4"><p className="text-sm font-bold text-slate-500">{filtered.length} {t.resultsNear} {location}</p><h2 className="text-3xl font-black text-[#002B5C]">{t.availableProducts}</h2></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{filtered.map((product) => <ProductCard key={product.id} product={product} lang={lang} onAdd={addToCart} />)}</div></section></section>
