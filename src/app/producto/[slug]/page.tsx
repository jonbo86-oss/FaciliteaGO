...
export default function ProductPage({ params }: { params: { slug: string } }) {
  const product = marketplaceProducts.find((item) => item.slug === params.slug) ?? marketplaceProducts[0];
    ...
    <main className="min-h-screen bg-[#f4f8ff] text-slate-950">
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-4 py-10 md:grid-cols-[1fr_0.9fr]">
        <div className="overflow-hidden rounded-[32px] bg-white shadow-2xl">
          <img src={product.imageUrl} alt={product.name} className="h-[430px] w-full object-cover" />
        </div>
        ...
        <article className="rounded-[32px] bg-white p-6 shadow-2xl md:p-8">
          ...
          <div className="mt-6 flex items-end gap-3">
            {product.oldPrice && <span className="text-lg font-bold text-slate-400 line-through">{money(product.oldPrice)}</span>}
            <span className="text-5xl font-black text-[#0072CE]">{money(product.price)}</span>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-12">
        <div className="grid gap-6 md:grid-cols-[1fr_360px]">
          <article className="rounded-[28px] bg-white p-6 shadow-xl">
            <h2 className="text-2xl font-black text-[#002B5C]">Descripcion y condiciones</h2>
            <ul className="mt-5 grid gap-3 md:grid-cols-2">
              {product.details.map((detail) => <li key={detail} className="rounded-2xl bg-blue-50 px-4 py-3 text-sm font-bold text-slate-700">{detail}</li>)}
            </ul>
          </article>
          <article className="rounded-[28px] bg-[#002B5C] p-6 text-white shadow-xl">
            <ShieldCheck className="h-8 w-8 text-[#FFCC00]" />
            <h2 className="mt-4 text-2xl font-black">Compra protegida</h2>
            <p className="mt-3 text-sm font-semibold leading-6 text-blue-100">Pago simulado para la demo, pedido trazable, comercio verificado y recogida con codigo.</p>
            <div className="mt-5 flex items-center gap-2 text-sm font-black"><Truck className="h-5 w-5" /> Recogida local prioritaria</div>
          </article>
        </div>
      </section>
    </main>
